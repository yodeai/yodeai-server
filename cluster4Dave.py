...
def clearDB():
    response, error = supabaseClient.table('block_for_dave').select("*").execute()    
    allRows = response[1]
    for row in allRows:        
        supabaseClient.table('block_for_dave').delete().eq("block_id", row["block_id"]).execute()
...
def makeDB():
    ...
    for index,key in enumerate(rawData):       
        ...
        supabaseClient.table('block_for_dave').upsert({
                'block_type': "text",
...
def loadData():
    data, error = supabaseClient.table('block_for_dave').select('block_id',  'summary', 'title', 'embedding','file_name', 'substantiveness').order('block_id').execute()
    ans = []
    count = 0
    for row in data[1]: 
        ...
        update_response, update_error = supabaseClient.table('block_for_dave')\
...
def addEmbeddings():
    data, error = supabaseClient.table('block_for_dave').select('block_id', 'summary', 'embedding').order('block_id').execute()
    ans = data[1]
    for index,element in enumerate(data[1]):
        print(f"getting embedding for {index}")
        element['embedding'] = getEmbeddings(element['summary'])
    supabaseClient.table('block_for_dave').upsert(ans, returning='minimal')
    return
...
def getClusterSummaryInfo(cluster, blocksInCluster):
    ...
    Then, in a new line write a summary for the entire text in at most 200 to 300 words that preserves concrete information such as names that are central to the narratives in the text.
