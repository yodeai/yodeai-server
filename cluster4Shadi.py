...
def getClusterSummaryInfo(cluster, blocksInCluster):
    ...
    # if (cluster == 4):
    #     clearConsole("printing cluster 4 info")
    #     for block in blocksInCluster:
    #         blockID = block['blockID']
    #         print(f"len of block b = {len(data[blockID]['summary'])}")
    #         print(f"title of block b is {(data[blockID]['title'])}")

    print(f"getting cluster summary for cluster {cluster}")
    text = ""
    promptNoText = f"""You are summarizing the following text inside qoutes in a concise way.     
    The text contains multiple parts, each about a document. Each parts is separated from the next part by two blank lines. 
    You are writing a single unified summary for the entire text.
    Firstly, find an informative title that reflects the main theme for the entire text. 
    Then, in a new line write a single summary for the entire text in at most 200 to 300 words that preserves concrete information that are central to the narratives in the text.
    ...
    promptLenWithoutText = len(promptNoText)

    for index, block in enumerate(blocksInCluster):
        newtext = data[block['blockID']]['summary'] + "\n\n\n"
        # if cluster == 4:
        #     print(f"inside for loop c4: len of block {index} is {len(newtext)}")
        if (len(text+newtext)+promptLenWithoutText>limits.maxChars4Prompt):
            print(f"gathered {index} blocks out of {len(blocksInCluster)}")
            break
        else:
            text += newtext
    ...
    # if (cluster > 30):
    #     print(f"cluster{cluster} has prompt:\n {prompt}")

    print(f"starting get_completion for cluster {cluster}")
    response = get_completion(prompt)
    title_starts = 1+response.find(":")
    title_ends = response.find("\n")    
    title = response[title_starts:title_ends].strip()
