...
folder_path = '../for-Stef/articles'
image_folders = ["Copy of Kolena Webinar - Oct 26 2023", "Copy of AI and UXR, Learners Event - Oct 25, 2023"]
...
def getSummaryData4RawDataBlock(text):
    if (len(text) > 15000): #on average the token limit is around 20000 words
...
def clearDB():
    response, error = supabaseClient.table('block_for_stef').select("*").execute()    
    allRows = response[1]
    for row in allRows:        
        supabaseClient.table('block_for_stef').delete().eq("block_id", row["block_id"]).execute()
...
def makeDB():
    ...
    for index,key in enumerate(rawData):       
        ...
        supabaseClient.table('block_for_stef').upsert({
                'block_type': "text",
...
def loadData():
    data, error = supabaseClient.table('block_for_stef').select('block_id',  'summary', 'title', 'embedding','file_name', 'substantiveness').order('block_id').execute()
    ans = []
    count = 0
    for row in data[1]: 
        ...
        update_response, update_error = supabaseClient.table('block_for_stef')\
...
def addEmbeddings():
    data, error = supabaseClient.table('block_for_stef').select('block_id', 'summary', 'embedding').order('block_id').execute()
    ans = data[1]
    for index,element in enumerate(data[1]):
        print(f"getting embedding for {index}")
        element['embedding'] = getEmbeddings(element['summary'])
    supabaseClient.table('block_for_stef').upsert(ans, returning='minimal')
    return
...
def getClusterSummaryInfo(cluster, blocksInCluster):
    ...
    relevantIndex = min(len(blocksInCluster), 8)
    ...
    # NOTE the promot below is auto-generated, and can be made more precise: it says the text contains multiple articles
    prompt = f"""You are summarizing the following text inside quotes in a concise way.
The text contains discussions of multiple articles. The discussions for consecutive article are seperated by two blank lines. 
    Firstly, find an informative title that reflects the main theme for the entire text. 
    Then, in a new line write a summary for the entire text preferably no more than 200 words that preserves concrete information that are central to the narratives in the text.
Text: "{text}" 
Your answer should be formatted as follows:
- In the first line, write "Title:" followed by the article's title.
- In the second line, write "Summary:" followed by the article's summary.
"""
...
themes = ["random text that means nothing", "user trust", "refining AI development process", "new products/approaches"]
#print(partitionBasedOnThemes(themes))

for index, entry in enumerate(data):
    print(f"theme for {entry['file_name']}:\n {assignTexttoTheme(entry['summary'], themes)}\n")
